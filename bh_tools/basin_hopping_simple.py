...
class BasinHoppingSimple:
    def __init__(self, system: Atoms, temp: float, hop_steps: int,
    ...
    def run_basin_hopping_sa(self):
        ...
        bh.run(steps=self.hop_steps)
        _, self.best_struc = bh.get_minimum()
        print("====== Basin Hopping Completed ======")
...
if __name__ == '__main__':
    ...
    view(Model)
    bh_sa = BasinHoppingSimple(system=Model, temp=1000, hop_steps=20, disturbance=0.3, f_max=0.10, step_max=300,
                               log_file_path="../Test/bh_sa.log",
                               potential_path="D:/fairchem_models/uma-s-1p1.pt",
                               job_type="omat")
